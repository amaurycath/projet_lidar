...
with open("coupe(h36,8;19,8;d.json", "r") as f:
    data = json.load(f)
...
phi = np.array([d["angle_moteur"] for d in data])

points = np.vstack((R, theta,phi)).T
"print(points[:400])"

test = points[:400]
print(test)

def autre_3D2(L, D=175, plot=True):
    "D=150 pour le pavé" 
    L = np.array(L)
    R = L[:, 0]
    theta = np.deg2rad(L[:, 1])
    phi = np.deg2rad(L[:, 2])
    
    X = (D - R*np.cos(theta)) * np.sin(phi)
    Y = -(D - R*np.cos(theta)) * np.cos(phi)
    Z = R * np.sin(theta)
    
    cartesian_points = np.column_stack((X, Y, Z))
    
    # Filtrage des points selon Z
    mask = (Y >= -100) & (Y <= 100) & (X>=-100) &(X<=100) & (Z>=-70) & (Z<=70)
